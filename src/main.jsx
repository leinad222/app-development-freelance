...
function ProductScene() {
    ...
    useEffect(() => {
        ...
        productGroup.rotation.set(-0.28, -0.5, 0.12);
        scene.add(productGroup);

        const bodyGeometry = new RoundedBoxGeometry(1.8, 0.12, 2.45, 8, 0.06);
        const bodyMaterial = new THREE.MeshStandardMaterial({ color: '#c9cbd0', roughness: 0.22, metalness: 0.85 });
        ...
        const intro = gsap.fromTo(productGroup.scale, { x: 0.35, y: 0.35, z: 0.35 }, { x: 1, y: 1, z: 1, duration: 1.4, ease: 'expo.out' });
        gsap.to(productGroup.rotation, { y: 0.1, z: -0.08, duration: 4, repeat: -1, yoyo: true, ease: 'sine.inOut' });
        gsap.to(productGroup.position, { y: 0.12, duration: 2.5, repeat: -1, yoyo: true, ease: 'sine.inOut' });
        let frameId;
        const render = () => {
        ...
        return () => {
            intro.kill();
            gsap.killTweensOf([productGroup.rotation, productGroup.position, productGroup.scale]);
...
const products = [
    ['Mac', 'From $799', 'product-mac', '⌘'],
    ['iPhone', 'From $599', 'product-phone', '●'],
    ['iPad', 'From $519', 'product-ipad', '▣'],
    ['Watch', 'From $299', 'product-watch', '◉'],
    ['AirPods', 'From $149', 'product-airpods', '◌'],
];

const services = [
    ['Expert service and support', 'From setup to screen repair, our specialists are here to help.', 'Learn more'],
    ['Free delivery in 60 minutes', 'Order online and get your essentials delivered when you need them.', 'Delivery details'],
    ['Buy online, pick up in-store', 'Reserve your order and collect it from your nearest Aster store.', 'Find a store'],
];
