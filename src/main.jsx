...
const fallbackImages = { iPhone: iphoneWhite, Mac: products[0][5], iPad: products[2][5], Watch: products[3][5], AirPods: products[4][5], 'TV & Home': products[0][5], AirTag: products[4][5], Accessories: products[2][5] };
...
function ProductCatalogue({ onAdded }) {
    ...
    return <div className="catalogue-shell">
        ...
        {!isLoading && !hasError && <><div className="catalogue-controls"><span>{visibleProducts.length} products</span><div><button onClick={() => scrollProducts(-1)} aria-label="Scroll products left">←</button><button onClick={() => scrollProducts(1)} aria-label="Scroll products right">→</button></div></div><div className="product-grid catalogue-grid" ref={productGridRef}>{visibleProducts.map((product) => { const fallback = fallbackImages[product.category] ?? iphoneWhite; return <article className={`product-card ${categoryClasses[product.category] ?? ''}`} key={product.id}><div className="product-image-wrap"><img src={product.image_url || fallback} onError={(event) => { if (event.currentTarget.src !== fallback) event.currentTarget.src = fallback; }} alt={product.name} loading="lazy" /><span className="product-mark">{categoryMarks[product.category] ?? '＋'}</span></div><h3>{product.name}</h3><p>{product.price}</p><button className="product-add" onClick={() => addProduct(product)} aria-label={`Add ${product.name} to bag`}>+</button></article>; })}</div></>}
